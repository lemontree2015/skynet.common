...
type Session struct {
	...
	sendMessageChan chan *gproto.Message // 直接发送
	...
	packetCallback     func(*Session, *gproto.Message)           // 接收Packet的回调
...
func NewSession(id uint64, server *Server, netConn *NetConn, sendChanSize uint) *Session {
	return &Session{
		...
		sendMessageChan:    make(chan *gproto.Message, sendChanSize),
		...
		if read_flag == readFlagStartPacket {
			...
			if packetSize == 0 {
				...
				if session.packetCallback != nil {
					iMessage := gproto.NewEmptyMessage()
					...
					if iMessage.MagicWord != gproto.MAGIC_WORLD {
						return
					}
...
// 注册packet callback.
func (session *Session) OnPacket(callback func(*Session, *gproto.Message)) {
...
// send binary
func (session *Session) SendMessage(iMessage *gproto.Message) error {
	if session.IsClosed() {
		return SendToClosedError
	}
